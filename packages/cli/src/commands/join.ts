...
export const joinCommand: Command = {
	name: 'join',
	description: 'Join an existing OmniMesh mesh as a QVAC consumer delegating to a provider.',
	usage:
		'join <providerPublicKey|omni://…> [--name=jeffs-mac] [--always-on-voice] [--secret=abc123]',
	run: ({ args, flags }) => {
		const raw = args[0]
		if (!raw) {
			console.error(`${c.red}✗${c.reset} provider public key required: omni join <pubkey|omni://…>`)
			return 1
		}
		...
		const child = spawn('bun', ['run', '--cwd', 'packages/host', 'src/join-consumer.ts'], {
			stdio: 'inherit',
			env: {
				...process.env,
