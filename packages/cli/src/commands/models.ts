...
async function listModels(ctx: CommandContext): Promise<number> {
	const provider = (ctx.flags.provider as string) ?? process.env.OMNI_PROVIDER_PUBLIC_KEY
...
async function pullModel(ctx: CommandContext): Promise<number> {
	...
	if (!alias) {
		console.error(`${c.red}✗${c.reset} alias required: omni models pull <alias>`)
		return 1
	}
	console.log(`${c.yellow}⚠${c.reset} Model pull not yet implemented — edit qvac.config.json serve.models and restart host`)
	return 0
}
...
async function deleteModel(ctx: CommandContext): Promise<number> {
	const alias = ctx.args[0]
	if (!alias) {
		console.error(`${c.red}✗${c.reset} alias required: omni models delete <alias>`)
		return 1
	}
	const secret = (ctx.flags.secret as string) ?? process.env.OMNI_SECRET
...
export const modelsCommand: Command = {
	name: 'models',
	description: 'Manage QVAC models via the model registry and OpenAI-compat server.',
	usage:
		'models <list|pull|run|delete> [alias] [--provider=<pubkey>] [--secret=abc123] [--prompt="..."]',
	run: async (ctx: CommandContext) => {
		const sub = ctx.args[0]
		if (!sub) {
			console.error(`${c.red}✗${c.reset} subcommand required: list|pull|run|delete`)
			return 1
		}
		switch (sub) {
			case 'list':
